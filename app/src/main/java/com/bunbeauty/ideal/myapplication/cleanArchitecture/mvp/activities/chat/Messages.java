    ...
    private void getMessages() {
        ...
        if (cursor.moveToFirst()) {
            ...
            do {
              ...
              /*  message.isCanceled = isCanceled;
                message.isMyService = isMyService;

                message.userId = senderId;
                message.serviceId = serviceId;
                message.userName = senderName;
                message.serviceName = cursor.getString(indexMessageServiceName);
                message.workingDay = date;
                message.workingTime = time;
                message.messageTime = WorkWithStringsApi.dateTimeToUserFormat(cursor.getString(indexMessageTime));

                message.reviewId = cursor.getString(indexMessageReviewId);
                ...
                if ((isAfterOrderTime(date, time) && !isCanceled) || (isCanceled && !review.equals("-"))) {
                    if (isMyService && type.equals(REVIEW_FOR_USER) || !isMyService && type.equals(REVIEW_FOR_SERVICE)) {
                        message.ratingReview = cursor.getString(indexMessageRatingReview);
                        message.type = type;
                        message.status = "no";
                    ...
                    if (isMyService && type.equals(REVIEW_FOR_USER) || !isMyService && type.equals(REVIEW_FOR_SERVICE)) {
                        message.workingTimeId = cursor.getString(indexMessageWorkingTimeId);
                        message.workingDayId = cursor.getString(indexMessageWorkingDayId);
                        message.orderId = cursor.getString(indexMessageOrderId);
